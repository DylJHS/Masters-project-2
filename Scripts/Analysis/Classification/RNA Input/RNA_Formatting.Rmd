...
tpm_samples_to_use <- order_tpm %>%  dplyr::select(c("Gene", ends_with(codes_to_use)))
```
...
```{r}
df_long <- pivot_longer(tpm_data[,0:10], cols = everything(), names_to = "Variable", values_to = "Value")
...
3.4. REMOVAL OF THE GENES THAT LACK EXPRESSION COUNTS 
