...
# final_cgbh <- adj_cgbh %>% 
#   group_by(seqnames) %>% 
#   mutate(pass = (width == max(width)) | (width >= 1000000)) %>% 
#   filter(pass) %>% 
#   mutate(index = cumsum(pass),
#          peri_id = ifelse(n()>1, 
#                           paste0("peri_",substring(as.character(seqnames), 4, nchar(as.character(seqnames))), "_",index),
#                           paste0("peri_",substring(as.character(seqnames), 4, nchar(as.character(seqnames))))
#                           )
#          ) %>% 
#    select(-c("pass","name"))
