...
plt_folder <- "Plots/Model_Plots/General_model/Results/"


# Set the order for the response features
response_features_order <- c(
  "1p", "1q", "2p", "2q", "3p", "3q", "4p", "4q", "5p", "5q","6p", "6q", "7p", "7q", "8p", "8q", "9p", "9q", "10p", "10q", "11p", "11q", "12p", "12q", "13q", "14q", "15q", "16p", "16q", "17p", "17q", "18p", "18q", "19p", "19q", "20p", "20q", "21q", "22q", "ai1", "lst1", "loh_hrd", "peri_1",
  "peri_2", "peri_3", "peri_4", "peri_5", "peri_7", "peri_8", "peri_9", "peri_10", "peri_16_1", "peri_16_2", "peri_17", "peri_20", "peri_22_3"
)
```
...
cat_cin <- read_tsv(
  ...
  replace(is.na(.), 0) %>%
  dplyr::select(-c("Type", "Aneuploidy Score")) %>%
...
labels <- full_data %>%
  dplyr::select(all_of(intersect(response_features, colnames(.)))) %>%
  mutate(
...
for (feature in response_features) {
  ...
  X <- full_df %>%
    dplyr::select(-c("Row.names", all_of(response_features))) %>%
    as.data.frame()
...
full_feature_imp <- read.csv(
  "/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Data/Gen_model_output/Results/Feature_importance/Full_base_feature_importance.csv",
  row.names = 1
)
...
#   filename = paste0(
#     plt_folder,
#     "Feature_importance/Base_Features_Heatmap.pdf"
#   ),
...
label_data <- base_oof_predictions %>%
  dplyr::select(starts_with("act_")) %>%
  dplyr::select(-act_index) %>%
...
full_feature_imp_df <- read.csv(
  "/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Data/Gen_model_output/Results/Feature_importance/Full_meta_feature_importance.csv",
  row.names = 1
)
...
ggplot2::ggsave(
  filename = paste0(
    plt_folder,
    "Feature_importance/Meta_Features_Heatmap.pdf"
  ),
  plot = heatmap_plot,
  width = 12,
  height = 9
...
```{r}
feature_order <- c("1p", "1q", "2p", "2q", "3p", "3q", "4p", "4q", "5p", "5q", "6p", "6q", "7p", "7q", "8p", "8q", "9p", "9q", "10p", "10q", "11p", "11q", "12p", "12q", "13q", "14q", "15q", "16p", "16q", "17p", "17q", "18p", "18q", "19p", "19q", "20p", "20q", "21q", "22q", "ai1", "lst1", "loh_hrd", "peri_1", "peri_2", "peri_3", "peri_4", "peri_5", "peri_7", "peri_8", "peri_9", "peri_10", "peri_16_1", "peri_16_2", "peri_17", "peri_20", "peri_22_3")
...
connect <- full_feature_imp_df %>%
  ...
  dplyr::select(from = Feature, to = Target, value = Gain)
...
ggsave(
  ...
  filename = "Plots/Model_Plots/General_model/Results/Feature_importance/Meta_Feature_Interdependencies.pdf",
  width = 12,
  height = 5
)
```
...
Cat_general_metrics <- Cat_general_metrics %>%
  ...
  dplyr::select(-Index) %>%
  distinct() %>%
  dplyr::select(Feature, Type, Accuracy, Kappa, AccuracyNull, AccuracyPValue)
...
```{r}
# Load the categorical metrics
Cat_general_metrics <- read.csv(
  "Data/Gen_model_output/Results/Gen_cat_metrics.csv",
  row.names = 1
)
...
cat_acc_plot <- ggplot(
  ...
  scale_color_manual(values = c("Base" = "#4d63b2", "Meta" = "#D21717")) +
  labs(
    title = "Accuracy of the Aneuploidy Learners",
    x = "Feature",
    y = "Accuracy",
    color = "Learner Type"
  ) +
...
# Save the plot
ggsave(
  filename = "Plots/Model_Plots/General_model/Results/Categorical/Model_Accuracy_line.pdf",
  plot = cat_acc_plot,
  width = 10,
  height = 5
)
...
reg_rsqd_plot <- ggplot(
  ...
  scale_color_manual(values = c("Base" = "#4d63b2", "Meta" = "#D21717")) +
  labs(
    title = expression("Pericentromeric and HRD Learners in terms of R"^2),
    x = "Feature",
    y = expression(R^2),
    color = "Learner Type"
  )+
...
# Save the plot
ggsave(
  filename = "Plots/Model_Plots/General_model/Results/Regression/Model_Rsquared_line.pdf",
  plot = reg_rsqd_plot,
  width = 10,
  height = 5
)
