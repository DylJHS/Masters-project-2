...
#factorise the colnames with levels
chr_cnv <- chr_cnv %>% 
...
by class
```{r}
class_metrics <- as.data.frame(confuse_matrix$byClass) %>% 
  rownames_to_column("Class") %>% 
  mutate(Class = as.integer(str_replace_all(Class, "Class: ", "")),
         Feature = target) %>%
...
class_metrics_long <- gather(class_metrics, key = Metric, value = Value, -Class, -Feature)

# Create bar plot comparing the metrics between the 3 classes
class_metrics_plot <- ggplot(class_metrics_long, aes(x = Metric, y = Value, fill = Class)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("#ABDDDE", "#CCEDB1", "#41B7C4"))+
  coord_flip() +  
  labs(title = "Class Metrics",
       subtitle = paste0("Feature: ", target),
       x = "Metric",
       y = "Value") +
  theme_minimal() +
  theme(axis.text.x = element_text( hjust = 1),  # Adjust text angle for better legibility
