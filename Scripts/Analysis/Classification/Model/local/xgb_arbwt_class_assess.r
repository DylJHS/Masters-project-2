...
target_weights <- c(0.00, 1.0, 0.00)
...
heatmap_plot <- ggplot(long_df, aes(x = Reference, y = Prediction, fill = NormalizedFreq)) +
  geom_tile(
    ...
    linetype = 1,
    lwd = 0.4
  ) + # Use white lines to separate the tiles
  ...
  labs(
    title = "Normalised Multi-class Confusion Matrix",
...
confusion_matrix_data <- as.data.frame(confuse_matrix$byClass)

# save the data
write.csv(confusion_matrix_data, paste0("Data/Model_output/Arbwtd_confusion_Matrix_", target, ".csv"))

# save the plot
ggsave(paste0("Data/Model_output/Arbwtd_confusion_Matrix_", target, ".png"), heatmap_plot, width = 10, height = 10, units = "in", dpi = 300)
