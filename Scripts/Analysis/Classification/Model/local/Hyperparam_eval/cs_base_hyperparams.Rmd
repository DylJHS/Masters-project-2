...
```{r}
hyper_folder <- "/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Data/Cancer_specific_data/Model_output/Hyperparameters/"

for (cancer_folder in list.files(hyper_folder)){
  cat(cancer_folder, "\n\n")
  sub_folder <- paste0(hyper_folder, cancer_folder, "/")
  
  
  cat_full_df <- data.frame()
  reg_full_df <- data.frame()
  
  for (file in list.files(sub_folder, pattern = "*.csv")){
    # Check the type of feature model (categorical or regression)
    Type = str_detect(file, "^\\d+")
    
    # Read the file
    current_file <- read.csv(paste0(sub_folder, file))
