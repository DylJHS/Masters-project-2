...
for (i in 1:length(rna_list)){
  ...
  for (feature in feature_list){
    ...
    for (depth in seq(3,8)){
      cat(paste0("\t Depth: ", depth, "\n"))
      for (lr in seq(0.06,0.1, by = 0.02)){
...
```{r}
min_metrics <- metrics_df %>%
  group_by(Feature) %>%
  summarise(Min = min(RSME)) %>%
  left_join(metrics_df, by = c("Feature", "Min" = "RSME"))

```

```{r}
# write.csv(metrics_df, "Data/gbm_metrics_params")
```
