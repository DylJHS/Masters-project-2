...
order_exp <- order_exp[0:35000,0:10000]
...
order_tpm <- ori_tpm[,order(colnames(ori_tpm))] %>% 
  dplyr::select(-"id")
order_tpm <- order_tpm[0:35000,0:10000]
...
full_df <- merge(log_exp, full_cin, by = "row.names")
...
y_train <- as.integer(train_set$loh_hrd)
X_train <- train_set %>% select(-append("Row.names",colnames(full_cin)))

y_test <- as.integer(test_set$loh_hrd)
...
print(paste("Best RMSE:", best_rmse, "at iteration:", best_iteration))
```

```{r}
xgb_preds <- predict(xgb_model, as.matrix(X_test), reshape = TRUE)
xgb_preds <- as.data.frame(round(xgb_preds)) %>% 
  rename(prediction = "round(xgb_preds)")
xgb_preds$actual <- y_test
xgb_preds$squared_error <- (xgb_preds$prediction - xgb_preds$actual)^2
