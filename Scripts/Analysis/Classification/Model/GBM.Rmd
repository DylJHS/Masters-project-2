...
order_tpm <- order_tpm[row_samples,0:2000]
order_exp <- order_exp[row_samples,0:2000]
...
```{r}
write.csv(reg_metrics_df, paste0("Data/reg_gbm_metrics_params_",Sys.Date(), ".csv"))
```

3.  Complete merge of all the predictor and the "categorical" response vars
```{r}
rna_list <- list(
  transcripts_per_million = tpm_set, # Seems to be the best performing
  scalled_transcripts_per_million = scld_tpm_set, # not too useful (scalled)
...
aneu_feature_list <- colnames(full_cin[1,6:7])

cat_metrics_df <- data.frame(
  RNA_Set = character(),
  Feature = character(),
  Depth = numeric(),
  Learning_Rate = numeric(),
  ...
  for (feature in aneu_feature_list){
    ...
    for (depth in seq(2,8,2)){
      for (gam in seq(0,4)) {
        ...
        # cat(paste0("the best loss is ", best_loss, "\n"))
        ...
        cat_metrics_df <- rbind(cat_metrics_df,
                                data.frame(
                                  RNA_Set = name,
                                  Feature = feature,
                                  Depth = depth,
...
min_metrics <- cat_metrics_df %>%
  group_by(Feature) %>%
  summarise(Min = min(Loss)) %>%
  left_join(cat_metrics_df, by = c("Feature", "Min" = "Loss"))

```
```{r}
write.csv(cat_metrics_df, paste0("Data/cat_gbm_metrics_params_",Sys.Date(), ".csv"))
```
...
#   "Aneuploidy Score"
# )
feature_list <- colnames(full_cin[1,6:7])
