...
for df in [GoI_RNA_df, GoI_CNV_df]:
	...
	while True:
		...
		if df == GoI_RNA_df:
			cntrl_df = pd.read_csv(f'../../Data/RNA_Data/Control_Genes_RNA_df{n}.csv')
		else:
			cntrl_df = pd.read_csv(f'../../Data/CNV_Data/Control_Genes_CNV_df{n}.csv')
		...
		print(reduced_full.iloc[:5,:5])


		# Save the datasets.

		# reduced_full.to_csv(f'../../Data/CNV/Full_{df}_Sample_metrics{n}.csv', index=False)
