...
```{r}
data <- fread("../../../Data/CNV_Data/Full_CNV_Sample_metrics.csv")
```
...
``` {r}
ggplot(order_data, aes(x = Sample, y = Median_diff, fill = TSS)) + 
  geom_col() +
  theme_minimal() +
  ylim(c(-0.5, 0.5)) +
...
```{r}
plt <- ggplot(results_mean_complete, aes(x = mean_diff, y = neg_log_pvalue, fill = TSS)) +
