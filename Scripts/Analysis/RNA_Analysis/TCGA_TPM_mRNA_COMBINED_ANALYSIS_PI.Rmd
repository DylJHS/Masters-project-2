...
# 
# 
# can_duo_table2 <- merge(can_duo_table, Meta_data[c("TSS.Code","Study.Name")],
#                     by = "TSS.Code", all = FALSE)
# nrm_duo_table2 <- merge(nrm_duo_table, Meta_data[c("TSS.Code","Study.Name")],
#                     by = "TSS.Code", all = FALSE)
...
# can_unique_Study <- unique(can_duo_table2$Study.Name)
# nrm_unique_Study <- unique(nrm_duo_table2$Study.Name)
# 
...
#   subset_data <- can_duo_table2[can_duo_table2$Study.Name == current_Study, ]
...
#   
#   # Subset the data for the current group
#   subset_data <- nrm_duo_table2[nrm_duo_table2$Study.Name == current_Study, ]
#   sample_num <- length(subset_data$SAMPID)
#   
#   # Perform the paired permutation test for the current group
#   test_result <- perm.test(subset_data$soi_geom_mean, subset_data$ctrl_geom_mean,
...
soi_combo <- merge(soi_merge,
                   Meta_data %>% select(c("TSS.Code", "Study.Name")),
                   by = "TSS.Code",
                   all = FALSE) %>% 
  select(-c("TSS.Code"))
...
F) Volcano plot of demonstrating the effect of the up-regulation in the SOI and how it compares to the other sets

```{r}
p2 <- ggplot(data=summary_pivot, aes(x=mean_log2fc, y=-log10(p))) + 
...
p2
```
Interpretation: The Volcano plot confirms that the set of interest demonstrates a significant and general up-regulation that varies greatly from the rest of the genome. 
...
- Perform the Wicoxon signed rank test between the two conditions
- Create the corresponding volcano plot and df
...
for (tissue in unique(all_combos$tissue_type)){
  ...
  assign(paste("p", tissue, sep = "_"),ggplot(data=tissue_summary, aes(x=avg_log2fc, y=-log10(p_val))) + 
   geom_point(aes(color = Tissue), alpha = 0.7) +
  geom_point(data = subset(
  tissue_summary, Set == "soi"),
  colour = "red",
  size = 3,
  shape = 1) +
  theme_minimal() + 
...
```{r}
p3 <- ggplot(data=all_tissue_summary, aes(x=avg_log2fc, y=-log10(p_val))) + 
   geom_point(aes(color = Tissue), alpha = 0.7) +
  # geom_point(color = "black") +
  theme_minimal() +
  geom_hline(yintercept=-log10(0.05), col="red") +
geom_point(data = subset(
  all_tissue_summary, Set == "soi"),
  colour = "red",
  size = 3,
  shape = 1
) +
...
#   geom_text(data = subset(tissue_summary, Set == "soi"), 
#             label = "Set of Interest",
#             hjust = 1, vjust = 2, color = "red", size = 4)
