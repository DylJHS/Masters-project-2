...
Obtain the Genes from the SOI (GOI)
```{r}
...
untransform <- function(x) {#Function to convert the log transformed counts back into orginal counts
  return((2^x)-1)
}
...
Edits ids so as to be able to identify the condition
```{r}
...
Create the linear model using the unique participants and the condition as the main effects.
```{r}
...
Get the table of all the genes and their fold changes and significances 
```{r}
