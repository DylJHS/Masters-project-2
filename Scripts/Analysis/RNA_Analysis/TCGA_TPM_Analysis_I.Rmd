...
for (i in 1:num_datasets) {
  ...
  ctrl_file <- paste("Data/RNA_Data/TCGA_TPM/TCGA_control_sets/TCGA_TPM_RNA_Control_df", i, ".csv", sep = "")
...
plot2 <- ggplot(graph_long_data, aes(y = Value, x = Variable)) +
  ...
  labs(x = "Set-of-Interest / Control-set Pair ", y = expression("Δ Expression" ~ (log[2])),
...
RNA_TSS <- ggplot(duo_table2, aes(x = factor(SAMPID), y = mean_dif)) + 
  ...
  facet_wrap(~Study.Name, ncol = 4, scales = "free_x", labeller = labeller(SMTS = function(label) label))  
...
# duo_table <- duo_table %>%
#   mutate( id = factor(id, levels = id[order(SMTSD)]))
``` 
...
unique_TSS <- unique(duo_table$SMTS_Abbrvs)
