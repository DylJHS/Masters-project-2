---
title: "RNA Healthy set Analysis PI"
...
Healthy_Set_of_Interest <- read.csv("Data/RNA_Data/GTEx_RNA/GTEx_RNA_Healthy/GTEx_RNA_SoI.csv")
Heatlhy_Sample_Meta <- read.csv("Data/RNA_Data/GTEx_RNA/GTEx_Annotations.csv")

```

Configure the set of interest metrics for the healthy data
```{r}
# use the log2 of the values
cols_to_log <- names(Healthy_Set_of_Interest)[-1]

Logs_Healthy_Set_of_Interest <- Healthy_Set_of_Interest %>% 
  mutate(across(.cols = cols_to_log, .fns = ~log2(. + 0.001), .names = "log2_{.col}")) %>% 
  select(1, starts_with("log2_"))

Summary_Healthy_SoI <- Logs_Healthy_Set_of_Interest %>%
