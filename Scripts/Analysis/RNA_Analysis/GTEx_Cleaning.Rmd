...
Metrics_table <- data.frame(SampleID = SOI_summary$SampleID)
...
for (i in 1:num_datasets) {
  ...
  Metrics_table <- merge(Metrics_table, ctrl_set_summary %>% select(SampleID , mean_dif, median_dif), by = "SampleID", all = FALSE, suffixes = c("", i))
...
violin_graph_long_data <- Metrics_table[dif_pairing_names] %>%
...
ggplot(violin_graph_long_data, aes(y = Value, x = Variable)) +
  ...
  labs(x = "Unique SOI to Control-set Pairings ", y = "Δ Log2 Expression",
