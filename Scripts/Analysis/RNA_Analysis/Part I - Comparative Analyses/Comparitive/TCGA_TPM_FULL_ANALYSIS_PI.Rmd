...
```{r}
numbers_list <- seq(1, 547)

# Load the data
summary_pivot <- read.csv("/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Data/RNA_data/Comparative_analysis_data/TCGA_full_Sets_Summary.csv")
...
pdf("/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Plots/RNA_Plots/RNA_Plts_Part_I/Sets Volcano Plots/Full_Volcano.pdf", width = 13, height = 11)
print(Full_Volcano)
dev.off()
...
# Save the data to a csv file
write.csv(all_tissue_summary, "/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Data/RNA_data/Comparative_analysis_data/TCGA_cancer_specif_summary.csv", row.names = FALSE)
...
Create the combined volcano plot only using the select tumour types,
highlighting the sets of interest  


```{r}
# Load the data
all_tissue_summary <- read.csv("/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Data/RNA_data/Comparative_analysis_data/TCGA_cancer_specif_summary.csv")

# Select the tumour types to be used
usable_tissues <- c("BRCA", "HNSC", "LUSC", "LUAD")

# Subset the data for the selected tumour types
usable_summary <- all_tissue_summary[all_tissue_summary$Tissue %in% usable_tissues,] %>% 
  merge(meta_data, by.x = "Tissue", by.y = "Study.Abbreviation", all = FALSE) %>% 
  dplyr::arrange(Set)

# factorize the set column
usable_summary$Set <- factor(usable_summary$Set, levels = c("soi", numbers_list))

# Create the colour key values for the plot
keyvals <- ifelse(
  usable_summary$Set == "soi", "#D21717", "#4d63b2")
keyvals[is.na(keyvals)] <- 'black'
names(keyvals)[keyvals == '#D21717'] <- 'Set of Interest'
names(keyvals)[keyvals == '#4d63b2'] <- 'Control Set'

# Create the size keyvals# Create the size keypals
keyvals3 <- ifelse(
  usable_summary$Set == "soi", 4.5, 2)

usable_volcano_facet <- EnhancedVolcano(usable_summary,
                                        lab = usable_summary$Set,
                                        x = "avg_log2fc",
                                        y = "p_val",
                                        selectLab = "None",
                                        title = "",
  ...
  facet_wrap(~Study.Name, ncol = 2, scales = "free")+
  theme(
...
# Save the plot
pdf("/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Plots/RNA_Plots/RNA_Plts_Part_I/Sets Volcano Plots/Usable_Volcano.pdf", width = 25, height = 19) 
print(usable_volcano_facet)
dev.off()
