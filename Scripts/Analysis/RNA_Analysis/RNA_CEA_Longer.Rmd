...
ggplot(main_data, aes(x = Sample, y = Mean_dif, fill = TSS_Abbrvs)) + 
  geom_col() +
  theme_minimal() +
...
C) Apply a two-sided paired permutation test between the SoI and the control set while grouping by cancer type in the form of TSS
...
Results_table <- data.frame(TSS = character(0), P_Value = numeric(0), Adj_P_Value = numeric(0), Significant_value= character(0))
...
for (i in 1:length(unique_TSS)) {
  ...
  Results_table <- rbind(Results_table, data.frame(TSS = current_TSS, P_Value = test_result$p.value, Adj_P_Value = Corrected_Pval,
                                                   Significant_value  = Significant_val)) %>% arrange(desc(P_Value))
}
