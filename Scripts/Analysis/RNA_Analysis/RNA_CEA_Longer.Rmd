...
Seeing as the Friedman test came back indicating that at least one of the controls shows a different expression distribution compared to the others, cannot sue the test as evidence to dismiss all controls but one. Instead we can visualise the general distribution acorsss all controls to determine if they act in a similar fashion. 
...
``` 
Interpretation: Despite that fact that the Friedman Test resulted in a significant result, the plot above clearly demonstrates that the differential expression pattern between every control set and the Set-of-Interest is bound in a narrow range centered between log2(2) and log2(3). Indicating the expression within the SoI is always increased compared 



D) Calculate the significance of the difference between the mean expression of both sets using a Wilcoxon Signed-Rank Test
