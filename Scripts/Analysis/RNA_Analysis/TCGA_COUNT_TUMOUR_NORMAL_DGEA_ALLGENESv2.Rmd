...
2.1. Re-transform the data from the y = Log2(x+1) values to the actual counts using x = 2^y^−1.
...
2.2. Convert the Gene ids into gene names
```{r}
...
2.3. Edit the samples (column names) into ids so as to be able to identify the participant and the cancer type
...
Convert data to a datatable for faster processing during the grouping of the duplicate genes
```{r}

setDT(counts_data)  # This modifies counts_data in place to a data.table
