...
```{r}
count_data_ordered <- soi[,order(colnames(soi))]
...
corr_matrix <- rcorr(log_data) # correlation matrix
...
myBreaks <- c(seq(-1, 0.8, length.out = 100), seq(0.81, 1, length.out = 100))
...
dist_matrix <- as.dist(1 - cor(log_data)) # dissimilarity matrix
dist_df <- as.data.frame(1 - cor(log_data)) # dissimilarity df
...
for (height in seq(1,length(node_heights),80)){
