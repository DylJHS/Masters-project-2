...
myBreaks <- c(seq(-1, 0.749, length.out = 250), seq(0, 1, length.out = 250))
...
# Get the resulting dendrogram
optimal_clusters <- cutree(dist_tree_xprss, h = optimal_node_ht)

# Get the genetic makeup of each cluster
optimal_clusters_df <- as.data.frame(optimal_clusters)%>%
  rownames_to_column("Gene") %>%
...
# Add this iteration's cluster information to the expression matrix
optimal_xprss_df_clustered <- left_join(log_data,
                                optimal_clusters_df,
                                by = "Gene") %>%
  select(-c("optimal_clusters","n_genes"))

# Group the expression matrix in terms of the clusters
optimal_grouped_xprss_clustered_df <- optimal_xprss_df_clustered %>%
  group_by(Cluster.Name) %>%
  summarise(across(-Gene, ~ mean(., na.rm = TRUE)))
```
...
```{r}
# Get the number of groups, clusters, unclustered genes & clustered genes
optimal_n_groups <- length(unique(optimal_clusters))
optimal_n_clusts <- optimal_clusters_df %>% 
  filter(n_genes > 1) %>% 
  pull(Cluster.Name) %>% 
  unique() %>% 
  length()
optimal_n_unclustered <- optimal_n_groups - optimal_n_clusts
optimal_n_clustered <- length(optimal_clusters_df$Gene) - optimal_n_unclustered


  # Plot the resulting tree
  plot(color_branches(dist_tree_xprss, h=optimal_node_ht),
       leaflab="none",
       col = paletteer_c("grDevices::Terrain 2",
                         length(unique(clusters))),
       main = " Hierarchical clustering (average method)"
       )
  abline(h = optimal_node_ht, col = "red", lwd = 0.75)
  mtext(paste0("Cut Height of ",round(optimal_node_ht, 2),
               ", yeilding ",
               optimal_n_clusts,
               " clusters"
               ),
