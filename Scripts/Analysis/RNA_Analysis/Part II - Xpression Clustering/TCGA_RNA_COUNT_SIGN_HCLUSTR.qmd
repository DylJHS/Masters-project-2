...
plot(color_branches(distance.tree, h=node_heights[cutheight]), 
     leaflab="none", 
     col = paletteer_c("grDevices::Terrain 2", length(unique(clusters)))
     )
```
...
# Initialize the df that will contain the coef Score, Node height, Number of groups formed
cluster_groups_df <- data.frame(Iteration = integer(),
                                Coef.Score = numeric(),
                                Node.Height = numeric(),
                                Cluster.Number = integer())

for (height in seq(1,length(node_heights)-1,5)){
  ...
  # Loop through each value and apply the given logic
  coef_sum <- function(x) {
    x_squared <- x^2
    non_neg <- x >= 0
  ...
  square_coef_score <- coef_sum(clustered_corr)
  ...
  # # Save the heatmaps
  # ggsave(paste0("clsutered_heatmap", height, ".png"),
  #      plot = clust_heatmap,
  #      width = 30,
  #      height = 25)
}
