...
Filter out the zero value genes and the non-SOI genes
```{r}
...
filtered_scaled <- reduced_scaled[rownames(reduced_scaled) %in% soi_genes, ]
```
...
```{r}
#transpose the data 
t_scaled_data <- t(filtered_scaled)
```


```{r}
dist_matrix <- t_scaled_data %>%
  dist() 
```
...
mds_df <- as.data.frame(mds) %>% 
  mutate(cancer_type = extract_element(rownames(.), 1),
         sample_type = extract_element(rownames(t_scaled_data), 3))
```
